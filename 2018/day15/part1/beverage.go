...
MainLoop:
	...
	fmt.Println("part1:", fullRound*totalHP)
}
...
func attack(unit *Unit, grid [][]*Node, units []*Unit) {
	...
	if unit.Pos.Y-1 >= 0 {
		node = grid[unit.Pos.Y-1][unit.Pos.X]
		if node.Unit != nil && node.Unit.Goblin != unit.Goblin && node.Unit.HitPoints < target.HitPoints {
			target = node.Unit
		}
	}

	if unit.Pos.Y+1 < gridY {
		node = grid[unit.Pos.Y+1][unit.Pos.X]
		if node.Unit != nil && node.Unit.Goblin != unit.Goblin && node.Unit.HitPoints < target.HitPoints {
			target = node.Unit
		}
